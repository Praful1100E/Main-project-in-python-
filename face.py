...
import os
import json
from tkinter import Tk, Label, simpledialog, messagebox, StringVar
...
# GUI Setup
root = Tk()
root.title("Face Recognition - Mobile Camera")
label = Label(root)
label.pack()

status_var = StringVar()
status_var.set("Waiting for face...")
...
def save_new_face(frame, face_location, face_encoding):
    ...
    mobile = simpledialog.askstring("New Face", "Enter mobile number:")
    ...
    if name and mobile:
        ...
        known_face_names.append(name)
        messagebox.showinfo("Saved", f"Face data for {name} saved successfully!")
...
def update_frame():
    ...
    if frame is None:
        status_var.set("Camera not available...")
    ...
    for face_encoding, face_location in zip(face_encodings, face_locations):
        ...
        cv2.putText(frame, name, (left + 6, bottom - 5),
